...
module.exports.read = (test) => {
  test('read', (t) => {
    ...
    t.equals(db.pragmas[0].sql, 'PRAGMA locking_mode=NORMAL')
    t.equals(db.pragmas[0].action, 'get')
    t.equals(db.pragmas[1].sql, 'PRAGMA synchronous=OFF')
    t.equals(db.pragmas[1].action, 'run')
    t.equals(db.pragmas[2].sql, 'PRAGMA mmap_size=268435456')
    t.equals(db.pragmas[2].action, 'get')
    t.equals(db.pragmas[3].sql, 'PRAGMA recursive_triggers=OFF')
    t.equals(db.pragmas[3].action, 'run')
